...
export default async function routes(app: FastifyInstance) {
  app.post('/bots', {
    ...
    const bot = await createBot(app.config.supabase, parsed.data as any)
    return bot
  })
  ...
  app.get('/bots/:id', {
    ...
    const bot = await getBot(app.config.supabase, id)
  ...
  app.get('/bots', {
    ...
    const bots = await listBots(app.config.supabase, ws)
    return bots
  })
  ...
  app.patch('/bots/:id', {
    ...
    const bot = await updateBot(app.config.supabase, id, patch)
    return bot
  })
  ...
  app.delete('/bots/:id', {
    ...
    const id = (req.params as any).id as string
    const ok = await deleteBot(app.config.supabase, id)
    return { ok }
  })
}