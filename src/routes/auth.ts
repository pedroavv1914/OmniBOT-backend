...
export default async function routes(app: FastifyInstance) {
  ...
  app.post('/auth/login', { schema: { tags: ['auth'], description: 'Login de usuário', security: [], body: { type: 'object', properties: { email: { type: 'string' }, username: { type: 'string' }, password: { type: 'string' } }, required: ['password'] }, response: { 200: { type: 'object', properties: { token: { type: 'string' } } } } } }, async (req, reply) => {
  ...
  app.post('/auth/register', { schema: { tags: ['auth'], description: 'Cadastro de usuário', security: [], body: { type: 'object', properties: { email: { type: 'string' }, username: { type: 'string' }, password: { type: 'string' } }, required: ['email','password'] }, response: { 200: { type: 'object', properties: { token: { type: 'string' }, user_id: { type: 'string' } } } } } }, async (req, reply) => {
    ...
    if (supabase && cleanEmail) {
      ...
      const t = data?.session?.access_token
      if (data?.user?.id) await createUser(supabase, { auth_user_id: data.user.id, email: cleanEmail })
      if (t) return { token: t }
